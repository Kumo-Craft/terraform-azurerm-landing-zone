...
## Control-plane diagnostic logs & cost

The module wires an `azurerm_monitor_diagnostic_setting` on the cluster when
`log_analytics_workspace_id` is set. Two inputs tune it:

| Variable | Default | Effect |
|----------|---------|--------|
| `diagnostic_log_categories` | the 6 historical categories (`kube-apiserver`, `kube-audit-admin`, `kube-controller-manager`, `kube-scheduler`, `cluster-autoscaler`, `guard`) | Categories enabled on the diagnostic setting. |
| `diagnostic_log_analytics_destination_type` | `null` (provider-managed / legacy **AzureDiagnostics**) | `"Dedicated"` routes logs to the resource-specific tables **AKSAudit / AKSAuditAdmin / AKSControlPlane**; `"AzureDiagnostics"` forces the legacy shared table. |

**Backward compatibility:** both defaults reproduce the previous behaviour — an
existing consumer that upgrades the module ref and changes nothing gets a
`No changes` plan.

**Cost lever (per [MS Learn](https://learn.microsoft.com/azure/aks/monitor-aks#azure-monitor-resource-logs)):**
`kube-audit-admin` is high-volume and irreducible at the source. To cut its cost,
set `diagnostic_log_analytics_destination_type = "Dedicated"` **and** put the
`AKSAuditAdmin` (and optionally `AKSControlPlane`) tables on the **Basic** plan via
the [`LogAnalyticsWorkspaceTable`](../LogAnalyticsWorkspaceTable) module (~0.65 $/GB
vs ~2.99 $/GB).

⚠️ **Switching to `Dedicated` is an in-place update of the diagnostic setting.** Data
already in `AzureDiagnostics` stays there and is **not** migrated — only new logs go
to the `AKS*` tables. Any existing KQL query / workbook / alert reading
`AzureDiagnostics` must be rewritten against the resource-specific tables.
