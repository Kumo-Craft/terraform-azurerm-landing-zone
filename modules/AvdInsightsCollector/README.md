...
It also creates the **Data Collection Endpoint** the agent needs to *fetch* that configuration when the workspace sits behind an AMPLS — see [Private Link](#private-link-the-dce).
...
> ⚠️ **`Memory` and `Terminal Services` take NO `(*)`.** They are single-instance
> Windows objects: `\Memory(*)\Available MBytes` is an **invalid path** and collects
> **nothing, silently**. Verified on a session host:
>
> ```
...
> ```
>
> Microsoft's counter list writes them with `(*)` — that page is descriptive, not
> literal DCR syntax (it also writes `Logical Disk` while the object is `LogicalDisk`).
> **Do not "fix" this back to match the docs**: these 7 counters collected nothing
> until 2026-09-03. A test guards it.
>
> Multi-instance objects (`PhysicalDisk`, `User Input Delay`, `RemoteFX Network`) DO
> require `(*)`. `RemoteFX Network` only yields data while a session is connected —
> empty on an idle host is expected.
...
## Private Link: the DCE

Behind an **AMPLS in `PrivateOnly`**, the Azure Monitor Agent cannot reach the default regional configuration endpoint. `GetConfig` loops forever on:

```
403 InvalidAccess — "Configuration access over Private Link requires using a data collection endpoint"
```

The agent then never learns *which* DCRs apply and ships nothing — while the extension still reports `Succeeded` and the `MonAgent*` processes stay alive. **A silent outage**: no `Heartbeat`, no `Perf`, no `Event`.

So this module creates a DCE (`public_network_access_enabled = false`, `kind` omitted — it qualifies OS-bound collection, not config access) and **one association per session host, named `configurationAccessEndpoint`**.

> **Why per-machine, and not `dataCollectionEndpointId` on the DCR?**
> Chicken and egg: the agent must reach the DCE **before** it knows any DCR. An endpoint referenced *inside* a rule can never be discovered by an agent that cannot read rules yet. Hence the association on the VM.
> — [MS docs: enable private link for monitoring VMs](https://learn.microsoft.com/azure/azure-monitor/fundamentals/private-link-vm-kubernetes)

> A DCE is a **network endpoint, not a destination**. Data still lands where `destinations.log_analytics` points, so adding one cannot reroute telemetry to another workspace.

### Two prerequisites outside this module

1. **Scope the DCE into the AMPLS** — `azurerm_monitor_private_link_scoped_service`, on the management subscription. Consume the `dce_id` output. **Without this the 403 persists**, module applied or not.
2. **Keep the subscription in `notScopes`** of any fleet-wide DCE association policy. A VM carries exactly **one** DCE association — a second one silently *replaces* the first, so policy and Terraform would fight on every compliance scan.

## Naming

| Resource | Pattern | Example | Override |
|---|---|---|---|
| DCR | `dcr-{acr}-{env}-{region}-{workload}-avdinsights` | `dcr-avd-prod-gwc-01-avdinsights` | `name` |
| DCE | `dce-{acr}-{env}-{region}-{workload}` | `dce-avd-prod-gwc-01` | `data_collection_endpoint_name` |

Both via the [`Naming`](../Naming/) submodule. The DCR shape mirrors AlzManagement's `dcr-mgm-prod-gwc-01-vminsights`. The DCE deliberately carries **no** `-avdinsights` component: the endpoint is regional and data-set agnostic — one DCE serves every rule in the region — unlike the rule it serves.
...
1. **Apply #1** — DCR + DCE created, no associations.
2. **Apply #2** — once the hosts are up, pass their ids → **two** associations per host: one to the DCR (what to collect) and one to the DCE (how to fetch the config).
...
| `data_collection_endpoint_name` | `string` | `null` | DCE name override. Null = derived. |
...
| `id` | DCR resource ID. |
| `name` | DCR name. |
| `association_ids` | Map of session host id → DCR association id (empty until `session_host_ids` is populated). |
| `dce_id` | **DCE resource ID — required by the platform to scope the DCE into the AMPLS.** |
| `dce_name` | DCE name. |
| `dce_configuration_access_endpoint` | Config access URL the agent calls to retrieve its DCRs. |
| `configuration_access_association_ids` | Map of session host id → `configurationAccessEndpoint` association id. |
...
- The **AMA extension** on the session hosts is out of scope (deploy it via policy/DINE or the VM extension) — this module only defines *what* to collect, *where* to ship it, and *how the agent fetches that configuration*.
- The AMA also needs a **usable identity**. A DINE parameterised with a placeholder user-assigned identity produces the same silent outage as a missing DCE: agent running, zero telemetry. Both must be right.

## Testing

`tests/basic.tftest.hcl` — plan-time, `mock_provider "azurerm"`: naming, `kind = Windows`, Perf/Event streams, the 15+5 counter split, the 6 event logs, associations per host, name overrides (DCR **and** DCE), lock, counter override, and validators (LAW id, empty counters/logs, sampling frequency bounds).
