...
| active_directory_administrators | Map of Entra admins (requires authentication.active_directory_auth_enabled). | `map(object)` | {} |
...
| active_directory_administrator_ids | Map of admin key => Entra administrator ID |
...
| active\_directory\_administrators | Map of Entra (Azure AD) administrators for the server. Requires authentication.active\_directory\_auth\_enabled = true. principal\_type: User \| Group \| ServicePrincipal. tenant\_id optional (defaults to authentication.tenant\_id, else the provider tenant). | <pre>map(object({<br>    object_id      = string<br>    principal_name = string<br>    principal_type = string<br>    tenant_id      = optional(string, null)<br>  }))</pre> | `{}` | no |
...
| Name | Description |
|------|-------------|
| active\_directory\_administrator\_ids | Map of administrator key => Entra administrator ID. |
