...
| allowed\_copy\_scope | Restricts where data can be copied FROM this account (`AAD` = same Entra tenant, `PrivateLink` = same private link scope). null leaves it unrestricted, which the ALZ guardrail deny-storage-copyscope flags. See the ../StorageAccount module. | `string` | `null` | no |
