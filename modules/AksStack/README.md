...
| Diagnostic Setting | 0–1 | Created by the wrapped Aks module when `log_analytics_workspace_id` is set. Tunable via the passthrough `diagnostic_log_analytics_destination_type` (default `null` = legacy AzureDiagnostics; `"Dedicated"` = resource-specific AKS* tables) and `diagnostic_log_categories` (default = 6 historical categories). See the [`Aks`](../Aks) diagnostics/cost section and the [`LogAnalyticsWorkspaceTable`](../LogAnalyticsWorkspaceTable) module for putting `AKSAuditAdmin` on the Basic plan. |
...
inputs = {
  ...
  # ── KV firewall (networkAcls) — optional, null by default ──────────
  # Brings the etcd-CMK Key Vault firewall under Terraform. Required as
  # default_action = "Deny" to satisfy Enforce-GR-KeyVaultSup0 (a distinct
  # property from public_network_access_enabled, which is already false here).
  # Omit it (null) to leave any existing/out-of-band networkAcls untouched —
  # azurerm_key_vault.network_acls is Optional+Computed, so null never resets it.
  kv_network_acls = {
    default_action = "Deny"
    bypass         = "AzureServices"
    # ip_rules   = ["203.0.113.10"]
    # subnet_ids = [dependency.network.outputs.subnet_ids["aks"]]
  }
...
| diagnostic\_log\_analytics\_destination\_type | Destination table layout for the AKS control-plane diagnostic setting. Either "Dedicated" (resource-specific tables AKSAudit / AKSAuditAdmin / AKSControlPlane — RECOMMENDED for new deployments per MS Learn CAF, and required to place kube-audit-admin on the Basic plan) or "AzureDiagnostics" (legacy shared table). Default null = provider-managed (keeps the existing behaviour — SWITCHING to Dedicated is an in-place change that sends NEW logs to the AKS* tables; data already in AzureDiagnostics stays there and existing KQL must be rewritten). | `string` | `null` | no |
| diagnostic\_log\_categories | AKS control-plane log categories enabled on the diagnostic setting. Default = the 6 historically-enabled categories (backward compatible). Note: kube-audit (full) is intentionally omitted in favour of kube-audit-admin; add it only if you accept its very high volume/cost. | `list(string)` | <pre>[<br>  "kube-apiserver",<br>  "kube-audit-admin",<br>  "kube-controller-manager",<br>  "kube-scheduler",<br>  "cluster-autoscaler",<br>  "guard"<br>]</pre> | no |
...
| kv\_network\_acls | Optional network ACLs (firewall) for the etcd CMK Key Vault — forwarded verbatim<br>to the ../KeyVault child's `network_acls` (same object shape as KeyVaultStack).<br>Null (default) omits the block.<br><br>NON-REGRESSION (important): `azurerm_key_vault.network_acls` is Optional+Computed<br>(verified in the provider source — v4.81.0 key\_vault\_resource.go: Optional: true,<br>Computed: true). With this null the KeyVault child emits NO network\_acls block, so<br>Terraform keeps whatever ACLs the vault already has — including any set out-of-band<br>(e.g. `az keyvault update`) — an apply does NOT reset an existing firewall.<br><br>Set it to bring the firewall under Terraform, e.g.<br>{ default\_action = "Deny", bypass = "AzureServices" } to satisfy<br>Enforce-GR-KeyVaultSup0 (which requires networkAcls.defaultAction = Deny, a<br>distinct property from public\_network\_access\_enabled). | <pre>object({<br>    default_action = string<br>    bypass         = string<br>    ip_rules       = optional(list(string), [])<br>    subnet_ids     = optional(list(string), [])<br>  })</pre> | `null` | no |
